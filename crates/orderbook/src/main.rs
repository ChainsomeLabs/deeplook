...
use deeplook_orderbook::cache::Cache;
...
async fn main() -> Result<(), anyhow::Error> {
    let Args { metrics_address, database_url, redis_url, rpc_url, env } = Args::parse();

    let mut db_connection = PgConnection::establish(&database_url.as_str()).expect(
        "Error connecting to DB"
    );
    let sui_client = SuiClientBuilder::default()
        .build(rpc_url.as_str()).await
    ...
    let current_checkpoint = CheckpointDigest::get_sequence_number(
        sui_client.clone().into()
    ).await.expect("Failed getting current checkpoint");

    println!("Current checkpoint: {}", current_checkpoint);

    let mut indexer = Indexer::new(
        database_url,
        DbArgs::default(),
        IndexerArgs {
            first_checkpoint: Some(current_checkpoint - 50),
