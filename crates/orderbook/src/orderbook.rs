...
use serde::Serialize;
use sui_sdk::{ rpc_types::{ SuiObjectData, SuiObjectDataOptions, SuiObjectResponse }, SuiClient };
use sui_types::{
...
use crate::{ cache::Cache, checkpoint::CheckpointDigest, error::DeepLookOrderbookError };
...
impl OrderbookManager {
    pub fn new(pool: Pool, sui_client: Arc<SuiClient>, cache: Mutex<Cache>) -> Self {
        ...
        OrderbookManager {
            ...
            checkpoint: None,
            sui_client,
            orderbook: Orderbook { asks: vec![], bids: vec![] },
            cache,
            price_factor,
            size_factor,
        }
    ...
    pub fn handle_update(&mut self, order: OrderUpdate) {
        ...
        match order.status {
            OrderUpdateStatus::Placed => self.add_order(order.price, order.quantity, order.is_bid),
