...
macro_rules! define_handler {
    {
        ...
        impl sui_indexer_alt_framework::postgres::handler::Handler for $handler {
            async fn commit<'a>(
                ...
                Ok(diesel::insert_into(deeplook_schema::schema::$table::table)
                    .values(values)
    ...
    };
    {
        ...
        db_model: $model:ty,
        table: $table:ident,
        tx_context: |$tx:ident, $checkpoint:ident, $env:ident| $ctx:expr,
        map_event: |$ev:ident, $meta:ident, $ctx_var:ident| $body:expr
    } => {
        pub struct $handler {
            env: $crate::DeepbookEnv,
        }
...
#[macro_export]
macro_rules! define_multi_handler {
    {
        name: $handler:ident,
        processor_name: $proc_name:literal,
        db_model: $model:ty,
            ...
            async fn process(
                ...
                for tx in &checkpoint.transactions {
                    ...
                    for (index, ev) in events.data.iter().enumerate() {
                        ...
                        $(
                            if !handled && <$event>::matches_event_type(&ev.type_, self.env) {
                                let $ev: $event = bcs::from_bytes(&ev.contents)?;
                                let $meta = base_meta.with_index(index);
                                results.push($body);
                                tracing::debug!("Observed {} event", $proc_name);
