...
async fn handle_orderbook_socket(mut socket: WebSocket, pool_name: String, state: Arc<AppState>) {
    ...
    let mut pubsub = cache.client.get_async_pubsub().await.expect("Failed getting pubsub");
    let channel = format!("__keyspace@0__:{}", redis_key);

    pubsub.subscribe(&channel).await.expect("Failed to subscribe to key‑space");

    // Helper to grab latest JSON
    let fetch_latest = || async {
        cache
            .get::<serde_json::Value>(&redis_key).await
            .ok()
            .flatten()
            .map(|v| v.to_string())
    };
